...
class V2X(pl.LightningModule):
    ...
    def __init__(
        ...
        # vehicle's view
        self.graph_car_view = SingleViewEncoder(
        ...
        # infrastructure's view
        self.graph_infra_view = SingleViewEncoder(
            historical_steps=historical_steps,
            node_dim=node_dim,
            edge_dim=edge_dim,
            embed_dim=embed_dim,
        ...
        # cross view (vehicle + infrastructure)
        self.graph_cross_view = CrossViewEncoder(
            historical_steps=historical_steps,
            embed_dim=embed_dim,
            edge_dim=edge_dim,
            num_modes=num_modes,
        ...
        # MLP decoder = generate multi-modal predicted trajectories
        self.decoder = MLPDecoder(
            single_view_channels=embed_dim,
    ...
    def forward(self, data: TemporalData):
        ...
        car_view_embed = self.graph_car_view(data=data, source=1) # vehicle's view
        infra_view_embed = self.graph_infra_view(data=data, source=0) # infrastructure's view
        cross_view_embed = self.graph_cross_view(data=data, car_view_embed=car_view_embed, infra_view_embed=infra_view_embed) # cross view
        y_hat, pi = self.decoder(single_view_embed=car_view_embed, cross_view_embed=cross_view_embed) # get predicted trajectories and corresponding probabilities
        return y_hat, pi
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument('--historical_steps', type=int, default=50) # past 5 seconds (historical horizons)
        parser.add_argument('--future_steps', type=int, default=50)     # prediction 5 seconds (future horizons)
