...
class SingleViewEncoder(nn.Module):
    """single-view encoder encodes single view (vehicle or infrastructure)"""
    ...
    def forward(self, data: TemporalData, source:int) -> torch.Tensor:
        #source 0-> infra;  1-> car
        source_mask = data.source==source
        edge_index, _ = subgraph(subset=source_mask, edge_index=data.edge_index)
        for t in range(self.historical_steps):
            data[f'edge_index_{t}'], _ = subgraph(subset=~data['padding_mask'][:, t], edge_index=edge_index)
            data[f'edge_attr_{t}'] = \
        ...
        out_temp = torch.zeros([out.shape[1],out.shape[2]], dtype=out.dtype, device=out.device)
        out_temp[source_mask, :]= self.temporal_encoder(x=out[:,source_mask,:], padding_mask=data['padding_mask'][source_mask][:, : self.historical_steps])
        edge_index, edge_attr = self.drop_edge(data['lane_actor_index'], data['lane_actor_vectors'])
        edge_index, edge_attr, _ = bipartite_subgraph(subset=(edge_index[0].unique(), source_mask), edge_index=edge_index, edge_attr=edge_attr, return_edge_mask=True)
        out = self.al_encoder(x=(data['lane_vectors'], out_temp), edge_index=edge_index, edge_attr=edge_attr,
...
class AAEncoder(MessagePassing):
    """agent-agent encoder captures relationships between agents and agents"""
...
class TemporalEncoder(nn.Module):
    """temporal encoder captures temporal relationships"""
...
class ALEncoder(MessagePassing):
    """agent-lane encoder captures relationships between agents and lanes"""
